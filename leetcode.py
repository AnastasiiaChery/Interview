...
Реалізуйте функцію myAtoi(string s), яка перетворює рядок у 
32-бітне знакове ціле число.

Алгоритм роботи myAtoi(string s) такий:

1. Пропуски: Ігноруйте всі початкові пробіли (" ").

2. Знак: Визначте знак числа, перевіривши, чи є наступний символ 
   '-' або '+'. Якщо жодного з них немає, вважаємо число додатним.

3. Перетворення: Зчитайте число, пропускаючи початкові нулі, 
   доки не зустрінеться нецифровий символ або не буде досягнуто 
   кінця рядка. Якщо жодної цифри не було зчитано, результат дорівнює 0.

4. Обмеження (округлення): Якщо число виходить за межі 
   32-бітного знакового діапазону [-2^31, 2^31 - 1], 
   потрібно обмежити його до цих меж:
   - якщо число менше за -2^31, повернути -2^31
   - якщо число більше за 2^31 - 1, повернути 2^31 - 1

Поверніть отримане ціле число.
...
Пояснення:
Підкреслені символи — це ті, які зчитуються, 
а символ ^ показує поточну позицію читання.

Крок 1: "42" (пробілів немає, нічого не пропускаємо)
         ^
Крок 2: "42" (немає ні '-' ні '+')
         ^
Крок 3: "42" (зчитується число "42")
"""

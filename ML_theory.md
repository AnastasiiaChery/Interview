...
# Справедливість: типи упередженості в машинному навчанні

Моделі машинного навчання **не є повністю об’єктивними**.
Вони навчаються на даних, які створюють і обробляють люди, а отже можуть містити **людські упередження**.

Розуміння цих упереджень допомагає:

* вчасно їх виявити
* зменшити їхній вплив
* зробити модель справедливішою

---

# Основні типи упередженості

## Упередженість звітування

**Що це:**
дані не відображають реальність, бо люди частіше фіксують **яскраві або крайні випадки**.

**Приклад:**
у відгуках про книгу більше:

* дуже позитивних
* або дуже негативних

→ модель гірше розпізнає **нейтральні думки**
...
**Що це:**
дані відображають **несправедливість минулого**.

**Приклад:**
дані про житло можуть містити:

* наслідки дискримінаційної політики
* нерівність у цінах

→ модель відтворює ці самі проблеми
...
**Що це:**
люди більше довіряють **автоматичним системам**, навіть якщо вони гірші.

**Приклад:**
модель знаходить дефекти гірше за людей,
але їй все одно більше довіряють.

---

# Упередженість відбору

Виникає, коли дані **не репрезентують реальний світ**.

## Упередженість охоплення

**Що це:**
частина важливої аудиторії **відсутня в даних**.

**Приклад:**
опитали лише покупців продукту,
але не тих, хто обрав конкурентів.
...
**Що це:**
деякі групи **рідше відповідають**, тому їх мало в даних.

**Приклад:**
покупці конкурентів частіше відмовляються від опитування.
...
**Що це:**
дані зібрані **невипадково**.

**Приклад:**
вибрали перших, хто відповів → це більш зацікавлені люди.

---

# Упередженість групової атрибуції

Це коли характеристики **окремих людей переносять на всю групу**.
...
**Приклад:**
розробники віддають перевагу кандидатам зі свого університету.
...
**Що це:**
спрощення — "усі вони однакові".

**Приклад:**
вважати, що всі кандидати без певної освіти — слабкі.
...
**Що це:**
рішення базуються на **особистому досвіді**, який не універсальний.

**Приклад:**
жест "так/ні" має різні значення в різних культурах.
...
**Що це:**
людина шукає або інтерпретує дані так, щоб **підтвердити свої переконання**.

**Приклад:**
ігнорування фактів, які суперечать гіпотезі.

---

# Висновок

Упередження в даних можуть:

* знижувати точність моделі
* створювати несправедливі рішення
* відтворювати дискримінацію

Тому важливо:

* уважно перевіряти дані
* аналізувати джерела упередженості
* тестувати модель на справедливість

👉 Навіть найкраща модель буде упередженою, якщо упереджені її дані.



# Об’єктивність: виявлення упередженості

Під час підготовки даних для машинного навчання важливо **заздалегідь виявляти упередженість**, щоб зменшити її вплив ще до розгортання моделі.

Нижче наведено основні **ознаки (червоні прапорці)**, які можуть свідчити про наявність упередженості в даних.

---

# Відсутні значення ознак

Якщо у даних є багато пропущених значень, це може означати, що:

* певні групи або характеристики **недостатньо представлені**
* модель буде гірше працювати для цих груп

---

## Приклад аналізу

Модель прогнозує, чи заберуть собаку з притулку.
...
У 1500 із 5000 прикладів **відсутній темперамент**.

---

## Потенційні джерела упередженості

### 1. Випадкові пропуски

* дані відсутні без системи
  → менший ризик, але все одно проблема якості

---

### 2. Пропуски за місцем (місто/село)

* наприклад, немає даних для собак із великих міст

⚠️ проблема:
місце може бути **прихованою ознакою (proxy)**

Наприклад:

* у містах частіше маленькі собаки
  → модель гірше працюватиме для них

---

### 3. Пропуски за віком

* відсутні дані для цуценят

→ модель буде менш точною для молодих собак

---

### 4. Пропуски за породою

* деякі породи не мають даних

→ модель буде несправедливою до цих порід

---

# Неочікувані значення ознак

Іноді дані містять **аномальні або нереалістичні значення**.
...
* лабрадору вказано **35 років**

⚠️ проблема:

* це нереалістично (максимум ≈ 29 років)
* ймовірно помилка (наприклад, 3.5 роки)
...
* спотворювати навчання
* погіршувати якість моделі
* створювати упередженість

---

# Спотворення даних

Упередженість виникає, якщо:

* деякі групи **перепредставлені**
* інші — **недопредставлені**
...
→ модель буде краще працювати для великих

---

# Як перевіряти справедливість моделі

Недостатньо оцінювати лише **загальну точність**.

Потрібно аналізувати результати окремо для:
...
* мати високу загальну точність
* але бути несправедливою для окремих груп

---

# Висновок

Щоб виявити упередженість:

* перевіряйте пропущені значення
* шукайте аномалії
* аналізуйте розподіл даних
* оцінюйте модель по підгрупах

👉 Справедливість моделі починається з якості даних.
